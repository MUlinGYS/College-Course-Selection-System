...
export function fetchRounds({ termId = '' } = {}) {
  const query = termId ? `?term_id=${termId}` : ''
  return apiGet(`/rounds${query}`)
}
...
export function fetchCourses({ q = '' } = {}) {
  const query = q ? `?q=${encodeURIComponent(q)}` : ''
  return apiGet(`/courses${query}`)
}
...
export function fetchSections({ termId = '', courseId = '', teacherId = '', q = '' } = {}) {
  const params = new URLSearchParams()
...
export function fetchMyEnrollments({ termId = '' } = {}) {
  const query = termId ? `?term_id=${termId}` : ''
  return apiGet(`/my/enrollments${query}`)
}
...
export function fetchMyConflicts({ termId = '' } = {}) {
  const query = termId ? `?term_id=${termId}` : ''
  return apiGet(`/my/conflicts${query}`)
}

export function fetchTeacherSections({ termId = '' } = {}) {
  const query = termId ? `?term_id=${termId}` : ''
  return apiGet(`/teacher/sections${query}`)
}

export function fetchTeacherRoster(sectionId) {
  return apiGet(`/teacher/sections/${sectionId}/roster`)
}

export function fetchEnrollmentRecords({ roundId = '', sectionId = '', studentId = '', status = '' } = {}) {
  const params = new URLSearchParams()
